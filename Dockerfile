...
# Install Neo4j 2.2.0 in /var/lib/neo4j
RUN mkdir /var/lib/neo4j
RUN wget http://dist.neo4j.org/neo4j-community-2.2.0-unix.tar.gz
RUN tar zxf neo4j-community-2.2.0-unix.tar.gz
RUN cp -r neo4j-community-2.2.0/* /var/lib/neo4j
...
# clean sources
RUN yum clean all
RUN rm neo4j-community-2.2.0-unix.tar.gz
RUN rm -rf neo4j-community-2.2.0
